...
/// Start a HEAD watcher for a repository via HTTP (browser-only mode).
pub(super) async fn start_head_watcher_http(
    ...
    match crate::head_watcher::start_watching(&q.path, app_handle.as_ref(), &state) {
        Ok(()) => (StatusCode::OK, Json(serde_json::json!({"ok": true}))).into_response(),
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e}))).into_response(),
    }
}

/// Stop a HEAD watcher for a repository via HTTP.
pub(super) async fn stop_head_watcher_http(
    State(state): State<Arc<AppState>>,
    Query(q): Query<PathQuery>,
) -> impl IntoResponse {
    if let Err(e) = validate_repo_path(&q.path) {
    ...
    crate::head_watcher::stop_watching(&q.path, &state);
