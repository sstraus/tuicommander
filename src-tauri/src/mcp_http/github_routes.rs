...
pub(super) async fn repo_all_pr_statuses(
    ...
    match tokio::task::spawn_blocking(move || crate::github::get_all_pr_statuses_impl(&paths, include_merged, &state)).await {
        Ok(Ok(statuses)) => Json(statuses).into_response(),
        Ok(Err(e)) => (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e}))).into_response(),
        Err(e) => (axum::http::StatusCode::INTERNAL_SERVER_ERROR, format!("Task failed: {e}")).into_response(),
    }
}
