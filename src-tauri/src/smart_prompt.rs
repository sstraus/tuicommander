...
mod tests {
    ...
    async fn headless_echo_command() {
        let result =
            execute_headless_prompt("echo hello".to_string(), 5000, "/tmp".to_string()).await;
    ...
    async fn headless_nonzero_exit() {
        let result =
            execute_headless_prompt("false".to_string(), 5000, "/tmp".to_string()).await;
    ...
    async fn headless_timeout() {
        let result =
            execute_headless_prompt("sleep 10".to_string(), 100, "/tmp".to_string()).await;
        assert!(result.is_err());
