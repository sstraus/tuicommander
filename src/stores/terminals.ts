...
export interface TerminalData {
  ...
  suggestDismissedAt: number; // Timestamp of last suggest dismiss (prevents re-show from parser re-emit)
}
...
function createTerminalsStore() {
  ...
  const actions = {
    /** Add a new terminal */
    add(data: Omit<TerminalData, "id" | "activity" | "progress" | "shellState" | "nameIsCustom" | "agentType" | "pendingResumeCommand" | "pendingInitCommand" | "usageLimit" | "lastDataAt" | "lastPrompt" | "agentIntent" | "currentTask" | "isRemote" | "agentSessionId" | "suggestedActions" | "suggestDismissedAt"> & { isRemote?: boolean }): string {
      const id = `term-${state.counter + 1}`;
      setState("counter", (c) => c + 1);
      setState("terminals", id, { id, activity: false, progress: null, shellState: null, nameIsCustom: false, agentType: null, pendingResumeCommand: null, pendingInitCommand: null, usageLimit: null, lastDataAt: null, lastPrompt: null, agentIntent: null, currentTask: null, isRemote: false, agentSessionId: null, suggestedActions: null, suggestDismissedAt: 0, ...data });
    ...
    register(id: string, data: Omit<TerminalData, "id" | "activity" | "progress" | "shellState" | "nameIsCustom" | "agentType" | "pendingResumeCommand" | "pendingInitCommand" | "usageLimit" | "lastDataAt" | "lastPrompt" | "agentIntent" | "currentTask" | "isRemote" | "agentSessionId" | "suggestedActions" | "suggestDismissedAt"> & { isRemote?: boolean }): void {
      setState("terminals", id, { id, activity: false, progress: null, shellState: null, nameIsCustom: false, agentType: null, pendingResumeCommand: null, pendingInitCommand: null, usageLimit: null, lastDataAt: null, lastPrompt: null, agentIntent: null, currentTask: null, isRemote: false, agentSessionId: null, suggestedActions: null, suggestDismissedAt: 0, ...data });
      if (data.sessionId) sessionToTerminal.set(data.sessionId, id);
    },
